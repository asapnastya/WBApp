...
final class MockChatData {
    ...
    private let system = MockUser(uid: "0", name: "System")
    ...
    private func randomImages() -> [MockImage] {
    ...
    private func randomMockImage() -> MockImage {
...
extension DraftMessage {
    ...
    private func makeMockImages() async -> [MockImage] {
        await medias
            .filter { $0.type == .image }
    ...
    private func makeMockVideos() async -> [MockVideo] {
        await medias
            .filter { $0.type == .video }
            .asyncMap { (media : Media) -> (Media, URL?, URL?) in
                (media, await media.getThumbnailURL(), await media.getURL())
